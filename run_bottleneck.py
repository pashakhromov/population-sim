...
scenario = 'bottleneck' # 'const', 'bottleneck', 'time_ss'
# bottleneck
Npop_lo = 267
Npop_hi = 1200
Ngen_lo = {
...
lists = (NetType, SampleSize, Theta, NatSel, RhoOverMu, np.arange(1, max(Nbatch.values())+1))
...
for element in itertools.product(*lists):
    net_type, sample_size, theta, nat_sel, rho_over_mu, batch = element
    Ns, Nds = nat_sel
    el = net_type, sample_size, theta, Ns, Nds, rho_over_mu, batch
    ...
    if net_type == 'FC' and rho_over_mu:
        ...
        pass
    elif batch > Nbatch[theta]:
        # so that always Nbatch * Nitr = 100
        pass
    else:
        # === create and populate folders ===
        dir_name = '{0}_n-{1:d}_theta-{2:0.1f}_Ns-{3:0.0f}-{4:0.0f}_RhoMuRatio-{5:0.0f}_batch-{6:d}'.format(*el)
        # create the folder
